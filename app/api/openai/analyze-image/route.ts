...
export async function POST(request: NextRequest) {
  const g = await guard(request, AnalyzeImageSchema, { bucket: 'analyze-image', capacity: 15, refillPerMinute: 15, requireAuth: true });
...
- Any nutritional information visible (calories, macros if on label)
...
Return JSON format:
{
  ...
  "macros": {
    "calories": number (optional, if visible on label),
